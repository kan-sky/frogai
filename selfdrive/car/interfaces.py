...
class CarStateBase(ABC):
  def __init__(self, CP):
    ...
    self.v_ego_clu_kf = KF1D(x0=x0, A=A, C=C[0], K=K) # use cluster speed
  ...
  def update_speed_kf(self, v_ego_raw):
    ...
    return float(v_ego_x[0]), float(v_ego_x[1]) # use cluster speed
  
  def update_clu_speed_kf(self, v_ego_raw): # use cluster speed
