...
from openpilot.selfdrive.car.gm.values import CAR, DBC, CanBus, CarControllerParams, CruiseButtons, GMFlags, CC_ONLY_CAR, EV_CAR
...
class CarController:
  ...
  @staticmethod
  def calc_pedal_command(accel: float, long_active: bool, Car_velocity) -> float: # modify for BOLT by geniuth
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 4 == 0:
        ...
        if not CC.longActive:
          ...
          self.apply_gas = self.params.INACTIVE_REGEN
          brake_step = int(min(1, self.CP.stoppingDecelRate * DT_CTRL * 4))
          self.apply_brake = int(max(self.apply_brake + brake_step, min(-100 * self.CP.stopAccel, self.params.MAX_BRAKE)))
        else:
          # Normal operation
